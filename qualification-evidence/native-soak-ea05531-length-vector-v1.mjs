...
if(!worker){
 ...
 const vectorProvider={kind:'local_onnx',provider_id:'soak.deterministic-fixture',model_id:'fixture-2d-v1',dimensions:2,timeout_ms:5000,async embed(texts){providerCalls++;providerItems+=texts.length;return texts.map(t=>Float32Array.from([t.length,t.charCodeAt(0)||1]));}};
 ...
 try{
  while(Date.now()<next.at){
   ...
   try {
   for(let q=0;q<5;q++){const start=performance.now();const result=await coordinator.search({query:'soakneedle000',limit:1});latencies.push(performance.now()-start);assert.equal(result.hits.length,1);assert.ok(result.hits[0].citation.matched_spans.some(s=>s.text==='soakneedle000'));const d=await engine.canonicalSha256(result.hits);if(resultDigest)assert.equal(d,resultDigest);resultDigest=d;}
   ...
   const p95=latencies.sort((a,b)=>a-b)[4];assert.ok(p95<plan.limits.queryP95Ms,'coordinator query p95 exceeded: '+p95);
