...
if(!worker){
 ...
 try{
  while(Date.now()<next.at){
   ...
   try {
   for(let q=0;q<50;q++){readMs=0;readCalls=0;const start=performance.now();const result=await coordinator.search({query:'soakneedle000',limit:1});latencies.push(performance.now()-start);sourceTimings.push({totalMs:latencies.at(-1),sourceReadMs:readMs,sourceReadCalls:readCalls});assert.equal(result.hits.length,1);assert.ok(result.hits[0].citation.matched_spans.some(s=>s.text==='soakneedle000'));const d=await engine.canonicalSha256(result.hits);if(resultDigest)assert.equal(d,resultDigest);resultDigest=d;}
