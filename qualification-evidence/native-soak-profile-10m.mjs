...
assert.equal(config.mode,'diagnostic');
assert.equal(config.durationMs,600000);
...
if(!worker){
 ...
 try{
  ...
  summary.status='DIAGNOSTIC_PASS';summary.cycles=checkpoint.cycle;
 ...
 try{
  while(Date.now()<next.at){
   ...
   let readMs=0,readCalls=0; const measuredReader=retrieval.vaultSourceReader(vault); const sourceTimings=[];
   const coordinator=new retrieval.RetrievalCoordinator(join(host.retrieval_directory.path,manifest.retrieval_projection_state.database_file),{discoverability_policy:()=> 'allow',source_discoverability_policy:()=> 'allow',source_reader:async p=>{const t=performance.now();try{return await measuredReader(p);}finally{readMs+=performance.now()-t;readCalls++;}},vector_provider:vectorProvider,runtime_policy_digest:digest,lineage_view_freshness:'fresh'});
   const latencies=[];let resultDigest=null;
   try {
   for(let q=0;q<50;q++){readMs=0;readCalls=0;const cpuStart=process.cpuUsage();const hostBefore=cpus();const at=new Date().toISOString();const start=performance.now();const result=await coordinator.search({query:'soakneedle000',limit:1});latencies.push(performance.now()-start);const ownCpu=process.cpuUsage(cpuStart);const hostAfter=cpus();const totals=hostAfter.reduce((a,c,i)=>{for(const k of Object.keys(c.times)){const d=c.times[k]-hostBefore[i].times[k];a.total+=d;if(k==='idle')a.idle+=d;}return a;},{total:0,idle:0});sourceTimings.push({q,at,totalMs:latencies.at(-1),sourceReadMs:readMs,sourceReadCalls:readCalls,ownCpuMs:(ownCpu.user+ownCpu.system)/1000,hostBusyFraction:totals.total?1-totals.idle/totals.total:null,hostFreeBytes:freemem()});assert.equal(result.hits.length,1);assert.ok(result.hits[0].citation.matched_spans.some(s=>s.text==='soakneedle000'));const d=await engine.canonicalSha256(result.hits);if(resultDigest)assert.equal(d,resultDigest);resultDigest=d;}
   ...
   latencies.sort((a,b)=>a-b);atomic(join(root,'query-samples-'+String(cycle+1).padStart(5,'0')+'.json'),{cycle:cycle+1,latenciesMs:latencies,watcherSearchMs,sourceTimings});const p95=latencies[47];assert.ok(p95<plan.limits.queryP95Ms,'coordinator query p95 exceeded: '+p95);
