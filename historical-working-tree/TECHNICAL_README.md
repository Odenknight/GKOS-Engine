...
sync-concurrency remain deferred. The approved 2.2 feature worktree adds an
...
adapter configuration and a current precondition provider. It journals intent,
archives exact before-images, writes same-directory temporary files, verifies
after-images, emits receipts, checkpoints shutdown, and blocks startup on
corrupt journal/archive bindings. Its capability remains false until the host
