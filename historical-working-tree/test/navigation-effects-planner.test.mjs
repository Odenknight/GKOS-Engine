...
test("path and agent grant validation rejects traversal, absolute, device, expiry, and collisions", () => {
  ...
  assert.deepEqual(resolveAgentNotePath({ grant, capability: "note:create", noteName: "daily.md" }), { valid: true, normalized: "_kosmos/agent-notes/atlas/daily.md", reasonCodes: [] });
  assert.equal(resolveAgentNotePath({ grant, capability: "note:archive", noteName: "daily.md" }).valid, false);
  assert.equal(resolveAgentNotePath({ grant, capability: "note:create", noteName: "../other/private.md" }).valid, false);
  assert.deepEqual(validateAgentGrant(grant, "2026-08-22T00:00:00Z"), ["GRANT_EXPIRED"]);
  assert.deepEqual(resolveAgentNotePath({ grant, capability: "note:create", noteName: "Daily.md", existingPaths: ["_kosmos/agent-notes/atlas/daily.md"] }).reasonCodes, ["PATH_CASE_OR_UNICODE_COLLISION"]);
});
