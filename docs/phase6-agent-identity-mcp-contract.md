...
Run `node scripts/generate-agent-identity-mcp-contract.mjs --check` to compare checked-in leaves with deterministic output. Run `node --test test/agent-identity-mcp-contract.test.mjs` for strict-schema, fixture closure, manifest integrity, reproducibility, archive, scope, protected-path, and secret-material checks. The generator's `--output-root <fresh-directory>` mode supports isolated-root reproducibility checks.
