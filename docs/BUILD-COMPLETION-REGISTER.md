...
| G3 managed ingestion and safety | Forty tests pass on local Python 3.14 and service Python 3.12; coverage includes eight process-exit transaction boundaries, twelve-generation revocation and late-phase ingestion quarantine; thirteen live checks pass | Backend/process orchestration, physical power-loss and complete production recovery qualification; ambiguous retries remain refused |
...
| G5 broker | Private broker/HTTP bounds pass; fresh ledger query boundary, late-response refusal and cancellation/outage cases included in forty passing host tests; thirteen live synthetic checks and four Python/TypeScript wire checks pass | Complete product-host scope authorization, authenticated product endpoint and production outage integration |
