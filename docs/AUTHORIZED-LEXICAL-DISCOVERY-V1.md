...
The service advertises this tool only when the host supplies the guarded retrieval validator. That validator applies the persisted source and whole-source chunk policies, checks UID/path, parser-fingerprint/digest, and public chunk identity conflicts, and returns the exact eligible source ID/path/digest set. The service refuses unless that set equals the authorized content scope; a missing indexed binding or policy-denied chunk cannot fall through to the content scan. The retained `VerifiedRetrievalSession.validateContentOnlyAuthorizedView` seam avoids reopening and revalidating the database for each page.
...
Lineage summaries use `currentness_contract_version: "observatory.lineage.v1"`. `authored_status` comes from the parsed `gkx.epistemic_state`, with explicit `status` retained separately when present; disagreement is preserved. `visible_lineage_status` is based only on resolved authorized lineage edges. An authored declaration with no visible endpoint returns `resolution_complete_within_scope: false`; hidden and physically absent endpoints remain observationally equivalent.
