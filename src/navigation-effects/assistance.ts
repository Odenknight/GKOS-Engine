...
function prose(raw: string): string {
  ...
  for (const line of content.split(/\r?\n/)) {
    ...
    if (!fence && !/^(?: {4}|\t)/.test(line)) lines.push(line);
  }
...
export async function buildDeterministicMocAssistance(input: MocAssistanceInput): Promise<MocAssistanceArtifact> {
  ...
  const bodies = new Map(state.notes.map(n => [n.uid, prose(n.content)]));
  const tags = state.notes.map(n => ({ uid: n.uid, tags: sorted([...n.tags, ...Array.from(bodies.get(n.uid)!.matchAll(/(?:^|\s)#([\p{L}\p{N}][\p{L}\p{N}_/-]{0,63})(?=\s|$|[.,;!?])/gu), m => tag(m[1]))]).slice(0, 20) }));
  ...
  for (const n of state.notes) {
    for (const match of bodies.get(n.uid)!.matchAll(/\[\[([^\]|#]+)(?:#[^\]|]*)?(?:\|[^\]]*)?\]\]/g)) {
