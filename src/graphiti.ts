...
export function buildGraphitiEpisodes(graph: GkxGraph, options: GraphitiOptions = {}): GraphitiEpisode[] {
  ...
  for (const node of graph.nodes) {
    ...
    output.push({
      ...
      source: "json",
      source_description: `GKX origin-separated source projection (${sourceOrigin}) · KGCP non-authoritative Graphiti adapter · workspace "${vault}" · ${node.path}`,
    ...
    for (const relationship of effectiveRelationshipEntries(node)) {
      ...
      output.push({
        ...
        source: "fact_triple",
        source_description: `Effective non-proposed GKX relationship projection from ${node.path}`,
        reference_time: timestamp,
        group_id: groupId,
