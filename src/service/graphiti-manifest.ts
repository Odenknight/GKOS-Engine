...
import { buildManagedGraphitiManifest, reconcileManagedGraphitiPublication } from "../graphiti-manifest";
...
export async function buildServiceGraphitiQueryContext(
  ...
  try {
    const verified = await reconcileManagedGraphitiPublication(() => buildServiceGraphitiManifest(input, sourceBytes),
      {...authority, policy_digest: input.authorization.policyDigest}, publication);
    if (!verified) return null;
    return {status:{contract_version:GRAPHITI_QUERY_CONTRACT_VERSION,mode:"managed",searchable:true,binding:verified.binding},
      decision:"allow",complete_dependency_scope:true,authorized_episodes:verified.episodes};
  } catch { return null; }
}
