...
test('two fresh roots and deterministic tar are byte-identical',async(t)=>{
  ...
  const ta=join(a,'pack.tar'),tb=join(b,'pack.tar'),receipt=join(a,'qualification-receipt.json'),evidence=join(a,'qualification-evidence.json');for(const [root,tar]of [[a,ta],[b,tb]])execFileSync(process.execPath,['scripts/generate-agent-identity-mcp-contract.mjs','--output-root',root,'--archive',tar,...(root===a?['--receipt',receipt,'--receipt-evidence',evidence,'--job','local-reproducibility','--receipt-command','deterministic local pack','--receipt-input',join(a,PACK,'pack-manifest.json'),'--receipt-input',join(a,PACK,'qualification-receipt.schema.json'),'--receipt-output',ta]:[])],{cwd:ROOT,stdio:'pipe'});
  ...
  assert.ok(value.cpu.length>0);assert.deepEqual(Object.keys(value.tool_versions).sort(),['ajv','ajv_formats','node','npm','sqlite','typescript']);assert.ok(value.commands.every((x)=>x.test_count===x.pass_count+x.fail_count+x.skip_count));assert.ok(value.input_artifact_digests.length>0);assert.ok(value.output_artifact_digests.length>0);assert.ok(Date.parse(value.ended_at)>Date.parse(value.started_at));
  for(const mutation of [(x)=>{delete x.cpu},(x)=>{x.tool_versions.npm='10'},(x)=>{x.tool_versions.sqlite='contract-only'},(x)=>{delete x.commands[0].test_count},(x)=>{x.input_artifact_digests=[]},(x)=>{x.output_artifact_digests=[]}]){const candidate=structuredClone(value);mutation(candidate);assert.equal(validate(candidate),false,JSON.stringify(candidate));}
  const synthetic=structuredClone(value);synthetic.commands[0].test_count=1;assert.equal(validate(synthetic),true);assert.equal(synthetic.commands.every((x)=>x.test_count===x.pass_count+x.fail_count+x.skip_count),false);
  const badLog=join(a,'synthetic.tap');await writeFile(badLog,'TAP version 13\n# tests 2\n# pass 1\n# fail 0\n# skipped 0\n');assert.throws(()=>execFileSync(process.execPath,['scripts/generate-agent-identity-mcp-contract.mjs','--output-root',a,'--check','--receipt',receipt,'--receipt-evidence',evidence,'--job','negative-synthetic-counts','--receipt-test-log',`synthetic::${badLog}`,'--receipt-input',join(a,PACK,'pack-manifest.json'),'--receipt-output',ta],{cwd:ROOT,stdio:'pipe'}),/Command failed/);
...
test('hosted workflow freezes all-and-only 11 jobs and artifacts',async()=>{
  const workflow=await readFile(join(ROOT,'.github/workflows/phase6-identity-contract.yml'),'utf8');assert.match(workflow,/^name: GKOS Phase 6 identity contract qualification$/m);
  ...
  assert.equal((workflow.match(/--receipt-evidence qualification-evidence\.json/g)||[]).length,11);assert.ok((workflow.match(/--receipt-input /g)||[]).length>=22);assert.ok((workflow.match(/--receipt-output /g)||[]).length>=11);
  assert.match(workflow,/--receipt-input-root predecessor-artifacts/);assert.match(workflow,/fs\.realpathSync\(os\.tmpdir\(\)\)/);assert.match(workflow,/manifest\.leaves\.length!==33/);assert.match(workflow,/synthetic command counts/);assert.doesNotMatch(workflow,/sqlite:'contract-only'|npm:'10'|input_artifact_digests:\[\]|output_artifact_digests:\[\]/);
});
