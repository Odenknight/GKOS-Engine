...
test('two fresh roots and deterministic tar are byte-identical',async(t)=>{
  ...
  const ta=join(a,'pack.tar'),tb=join(b,'pack.tar'),receipt=join(a,'qualification-receipt.json');for(const [root,tar]of [[a,ta],[b,tb]])execFileSync(process.execPath,['scripts/generate-agent-identity-mcp-contract.mjs','--output-root',root,'--archive',tar,...(root===a?['--receipt',receipt,'--job','local-reproducibility']:[])],{cwd:ROOT,stdio:'pipe'});
  ...
  const ajv=new Ajv2020({strict:true,allErrors:true,allowUnionTypes:true});addFormats(ajv);assert.equal(ajv.validate(await json('qualification-receipt.schema.json'),JSON.parse(await readFile(receipt,'utf8'))),true,ajv.errorsText(ajv.errors));
