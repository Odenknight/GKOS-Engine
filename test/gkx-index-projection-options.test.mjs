/**
 * GkxIndex projection-options threading (issue #6).
 *
 * Proves Gkx23ProjectionOptions.defaultSensitivity reaches every internal
 * buildGkx23Projection call through the indexing path — full load AND
 * incremental reparse — while an omitted option keeps the fail-closed
 * "secret" default unchanged.
 */
import test from "node:test";
import assert from "node:assert/strict";
import { GkxIndex, buildGraph, parseSourceFile } from "../dist/gkos-engine.mjs";

const N = (path, content) => ({ relativePath: path, content, kind: "note" });
// An GKX note with frontmatter present (so a projection is built) but NO
...
const effectiveSensitivity = (idx, path) =>
  idx.graph.nodes.find((n) => n.id === `file:${path}`)?.gkx?.projection?.effective.sensitivity;

test("GkxIndex built with defaultSensitivity=internal yields internal for an unlabeled note", () => {
  const idx = new GkxIndex({ defaultSensitivity: "internal" });
...
test("buildGraph and parseSourceFile forward the option; omitted stays secret", () => {
  ...
  assert.equal(g.nodes.find((n) => n.id === "file:A.md").gkx.projection.effective.sensitivity, "restricted");
  assert.equal(buildGraph([N("A.md", UNLABELED)], []).nodes.find((n) => n.id === "file:A.md").gkx.projection.effective.sensitivity, "secret");

  const rec = parseSourceFile(N("A.md", UNLABELED), { defaultSensitivity: "confidential" });
  assert.equal(rec.gkx.projection.effective.sensitivity, "confidential");
  assert.equal(parseSourceFile(N("A.md", UNLABELED)).gkx.projection.effective.sensitivity, "secret");
});
